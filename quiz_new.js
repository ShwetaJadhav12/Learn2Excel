...
function selectOption(index) {
    selectedAnswers[currentQuestionIndex] = index;
    localStorage.setItem('selectedAnswers', JSON.stringify(selectedAnswers)); // Save answers to localStorage
...
function submitQuiz() {
    ...
    // Clear localStorage after quiz submission
...
async function fetchHistory() {
    ...
    if (history.length === 0) {
        ...
        history.forEach((entry) => {
            ...
            listItem.innerHTML = `
                <strong>Score:</strong> ${entry.score}/${entry.total_questions} <br>
